...
const getShapeMix = (gender, newgeneration) => {
	if (newgeneration) return gender ? randomHelper(5, 10) : randomHelper(0, 5)
	else return gender ? 7 : 2
}
...
const CreateNewCustomization = (gender, newgeneration = false) => {
	...
	appearancesList.forEach((component) => {
		if (component.onlysex === 'all' || component.onlysex === gender) {
			...
			if (key === 'hair' || key === 'eyebrows' || key === 'facialhair' || key === 'molesfreckles' || key === 'eyescolor') id = randomHelper(0, appearance.length)
