...
accountData.subscribe((value) => {
	if (value.charsSlot) chars = value.charsSlot
})
...
export const updateGender = async (newgender) => {
...
export const updateParents = (gender, mother, father) => {
	...
	if (init) executeClient('client.characters.customization.UpdateParents', gender, parents[false][mother].gtaId, parents[true][father].gtaId)
	updateCustomizationData()
}
...
export const updateCharacteristic = (gender, key, preset, xvalue, yvalue) => {
	...
	if (init) executeClient('client.characters.customization.UpdateCharacteristic', gender, indexes.xindex, xvalue, indexes.yindex, yvalue)
...
export const updateAppearance = (gender, key, id, color, opacity) => {
	...
	if (init) executeClient('client.characters.customization.UpdateAppearance', gender, key, appearance[id].id, color, opacity)
...
export const updateClothes = (gender, key, newcloth) => {
	...
	if (init) executeClient('client.characters.customization.UpdateClothes', gender, key, cloth.drawable, cloth.texture)
