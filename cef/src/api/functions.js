...
window.listernEvent = (eventName, ...args) => {
	try {
		if (typeof window.functionList[eventName] === 'function') window.functionList[eventName](...args)
	} catch (err) {
	...
	const updateNode = (node, url) => {
		if (url && /(?:jpg|jpeg|png)/g.test(url)) {
			...
			img.onload = () => {
				...
				return node.nodeName === 'IMG' ? (node.src = image) : (node.style.backgroundImage = `url(${image})`)
			}
		}
	}
