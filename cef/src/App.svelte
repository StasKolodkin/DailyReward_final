<script>
	...
	document.cloud = 'https://cdn.gta5aqua.com/cloud/'
	...
	import '@aquafonts/stylefont.css'

	import '@store/account'
	import { charIsPet } from '@store/chars'
	import '@store/loader'
	import '@store/keys'
	import '@store/equipments.js'
	import { inVehicle, isInputToggled } from '@store/hud'
	import '@store/quest'
	import '@store/settings'

	import { executeClient, isCodeSetted } from '@api/rage'
	...
	// import PlayerJailmenu from '@views/player/jailmenu/index.svelte'

	import DailyReward from "./components/DailyReward/index.svelte"
	...
	//Popups
	// import Anim from '@popups/Anim/index.svelte'
	...
	onMount(async () => {
		while (!isCodeSetted()) { await window.sleep(1); }
		executeClient('client:OnBrowserInit')

		if (!isMultiplayer) {
			//window.router.setHud();
		//window.router.updateStatic("BusinessMenu");
		 //   window.router.setView("BusinessMenu");

			window.router.setView("NameAppMenu");
			...
			// for (let i = 0; i < 9; i++) window.chat.addMessage(i + ' test тектс тектс тектс тектс тектс тектс тектс тектс тектс тектс тектс тектс тектс тектс тектс тектс')
	...
	const handleKeydown = (event) => {
		...
		else if ($inVehicle || $isInputToggled) return
		const { keyCode } = event

		if (isKeyBind !== -1) {
			if (isKeyBind === $keys[55] && keyCode === $keys[55]) {
...
<FadeContainer />
<ViewContainer visible={!!Popus[$router.popup]} opacity={$router.opacity && $router.popup !== 'PopupCamera' ? 1 : 0}>
	{#if Views[$router.view]}
		<svelte:component this={Views[$router.view]} viewData={$router.viewData} />
	{/if}
	<!-- <PlayerInventory visible={$router.PlayerInventory} /> -->
